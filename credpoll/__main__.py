...
def list_accounts(logs_dir: str) -> list:
    ...
    for hash in hashes:
        ...
        if identity.lower() not in map(
            str.lower, new_accounts
        ) and identity.lower() not in map(str.lower, CACHE):
...
def extract_creds(logs_dir: str) -> str:
    ...
    for hash in hashes:
        if (
            hash.split(":")[0] == extract_account.split("\\")[1]
            and hash.split(":")[2] == extract_account.split("\\")[0]
        ):
            return hash
